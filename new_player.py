...
class Player:
    def __init__(self):
        ...
        self.x, self.y = self.WIDTH // 2, self.HEIGHT - 100
    ...
    def update(self, platform_rects, wall_rects):
        self.update_physics(platform_rects, wall_rects)
    ...
    def check_wall_collision(self, wall_rects):
        self.on_wall = False
        for wall_rect in wall_rects:
            if pygame.Rect(self.x, self.y, self.width_player, self.height_player).colliderect(wall_rect):
                self.on_wall = True
                self.speed_y = min(self.speed_y, int(self.WALL_SLIDE_SPEED))  # Применение эффекта "скольжения"
                print("Игрок касается стены!")
                break
    ...
    def handle_keyboard_input(self):
        ...
        if not self.on_wall:  # Проверка, что персонаж не касается стены
            if keys[pygame.K_a]:
                self.x -= 5
            if keys[pygame.K_d]:
                self.x += 5
    ...
    def update_physics(self, platform_rects, wall_rects):
        self.gravity_and_verticalmovement()

        if not self.check_platform_collision(platform_rects):
            self.check_ground_collision()

        self.check_wall_collision(wall_rects)
    ...
    def check_platform_collision(self, platform_rects):
        for platform_rect in platform_rects:
            # Проверка столкновений с платформой снизу (по оси Y)
            if pygame.Rect(self.x, self.y + self.height_player, self.width_player, 1).colliderect(platform_rect):
                self.y = platform_rect.top - self.height_player  # Устанавливаем персонажа на платформу
                self.speed_y = 0  # Останавливаем вертикальное движение
                self.on_ground = True  # Игрок на земле
                return True

            # Проверка столкновений с платформой сбоку (по оси X)
            if pygame.Rect(self.x, self.y, self.width_player, self.height_player).colliderect(platform_rect):
                # Если персонаж стоит на платформе и его бок сталкивается с платформой
                if self.speed_x > 0:  # Двигается вправо
                    self.x = platform_rect.left - self.width_player  # Устанавливаем персонажа слева от платформы
                elif self.speed_x < 0:  # Двигается влево
                    self.x = platform_rect.right  # Устанавливаем персонажа справа от платформы
...
for layer in tmx_data.layers:
    if layer.name == 'Walls':
        for x, y, surf in layer.tiles():
            pos = x * tmx_data.tilewidth, y * tmx_data.tileheight
            wall_rect = pygame.Rect(pos[0], pos[1], tmx_data.tilewidth, tmx_data.tileheight)
...
while running:
    ...
    player.update(platform_rects, wall_rects)
